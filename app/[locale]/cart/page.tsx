...
export default function CartPage() {
  ...
  // حساب الإجماليات بالأسعار المحسوبة
  const subtotal =
    cartData?.data?.cart?.reduce(
      (sum: number, item: any) =>
        sum + calculateLivePrice(item.product) * Number(item.quantity || 1),
      0
    ) || 0;
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-0">
                        ...
                        <div className="font-sukar font-semibold text-md flex items-center gap-1">
                          {calculateLivePrice(item.product).toLocaleString(
                            "en-US",
                            {
                          ...
                          {
                            calculateLivePrice(item.product) !==
                              Number(
